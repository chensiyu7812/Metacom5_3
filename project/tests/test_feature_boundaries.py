...
def test_ablation_dimensions_are_distinct(tiny_state):
    dims={}
    for mode in ('text_only','metadata_only','text_metadata','text_metadata_stable','catalog_only','full'):
    ...
    assert dims['text_metadata_stable']==dims['text_metadata']
...
def test_stable_metadata_caps_external_scale(tiny_state):
    b=FeatureBuilder(mode='text_metadata_stable').fit([tiny_state])
    shifted=tiny_state.model_copy(deep=True)
    shifted.session_index=1000
    first = next(iter(shifted.inventory))
    shifted.inventory[first] = shifted.inventory[first].model_copy(
        update={"count": 1000, "estimated_tokens": 100000, "max_age_sessions": 1000}
    )
    report=b.ood_report([(shifted,'M0+R0')])
    assert report['severe_scalar_ood'] is False
