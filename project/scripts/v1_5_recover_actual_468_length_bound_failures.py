...
def main() -> None:
    ...
    recovery_report = {
        ...
        "note": (
            ...
            "finish reason; every other schema constraint (verdict enum, <=4 "
            "evidence items, non-empty, aligned lists) was re-verified, not "
            "bypassed. Citation-integrity defects are disclosed honestly, "
            "never upgraded to valid: report-only-not-outcome-v1 policy "
            "already governs citation_valid for this stage and is unchanged "
            "by this recovery. Zero new API calls; zero new cost."
        ),
