#!/usr/bin/env python3
"""Run the exact 102-call fresh-identity RS+MS quality/risk blind measurement."""
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_quality_risk_measurement_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_quality_risk_measurement_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_ms_quality_risk_measurement_endpoints_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_quality_risk_measurement_preflight_20260812"
PLAN = PREFLIGHT / "call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_quality_risk_measurement_live_20260812"
USD_CAP = 1.2
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_QUALITY_RISK_MEASUREMENT_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("RS+MS quality/risk measurement execution is not current")
    ...
    if bundle["current_phase"]["id"] != "RS_MS_QUALITY_RISK_MEASUREMENT_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    if len(calls) != 102 or len({row["logical_call_id"] for row in calls}) != 102:
        raise RuntimeError("exact 102-call plan required")
    for call in calls:
    ...
    dry = {
        "status": "LIVE_READY_EXACT_102_RS_MS_QUALITY_RISK_CALLS",
        "logical_calls": 102,
        "maximum_physical_attempts": 204,
    ...
    try:
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                ...
                raw.append({
                    "protocol": "pm-v1.5-paper1-rs-ms-quality-risk-measurement-raw-v1",
            ...
            results.append({
                "protocol": "pm-v1.5-paper1-rs-ms-quality-risk-measurement-result-v1",
                "logical_call_id": call["logical_call_id"],
                "kind": call["kind"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"RS+MS quality/risk measurement {len(results)}/102 kind={call['kind']} valid={validated is not None}", flush=True)
    finally:
    ...
    complete = len(results) == 102
    report = {
        "protocol": "pm-v1.5-paper1-rs-ms-quality-risk-measurement-live-report-v1",
