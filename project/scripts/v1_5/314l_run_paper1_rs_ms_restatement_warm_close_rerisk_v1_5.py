#!/usr/bin/env python3
"""Run the exact 31-call fresh-identity restatement warm-close rerisk measurement."""
...
from metacom_pm.v1_5_rs_ms_quality_risk_blind_review import RSMSRiskBlindReview  # noqa: E402
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_restatement_warm_close_rerisk_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_restatement_warm_close_rerisk_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_ms_quality_risk_measurement_endpoints_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_rerisk_preflight_20260812"
PLAN = PREFLIGHT / "call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_rerisk_live_20260812"
USD_CAP = 0.5
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_RERISK_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("restatement warm-close rerisk execution is not current")
    if bundle["current_phase"]["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_RERISK_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    if len(calls) != 31 or len({row["logical_call_id"] for row in calls}) != 31:
        raise RuntimeError("exact 31-call plan required")
    for call in calls:
        if call["messages_sha256"] != sha256_text(canonical_json(call["messages"])):
            raise RuntimeError(f"prompt drift: {call['logical_call_id']}")
        if call["schema_sha256"] != sha256_text(canonical_json(RSMSRiskBlindReview.model_json_schema())):
            raise RuntimeError("schema drift")
    dry = {
        "status": "LIVE_READY_EXACT_31_RESTATEMENT_WARM_CLOSE_RERISK_CALLS",
        "logical_calls": 31,
        "maximum_physical_attempts": 62,
        ...
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                result, parsed = clients[endpoint_key].chat(
                    ...
                    seed=int(call["seed"]),
                    response_schema=RSMSRiskBlindReview,
                    retries=2,
                )
                raw.append({
                    "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-rerisk-raw-v1",
                ...
                if parsed.item_id != call["item_id"]:
                    raise ValueError("item_id_mismatch")
                validated = parsed.model_dump(mode="json")
            except Exception as exc:
                error = f"{type(exc).__name__}: {exc}"
            results.append({
                "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-rerisk-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"restatement warm-close rerisk {len(results)}/31 valid={validated is not None}", flush=True)
    finally:
    ...
    complete = len(results) == 31
    report = {
        "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-rerisk-live-report-v1",
        "status": "RERISK_CALLS_COMPLETE_ZERO_API_ANALYSIS_NEXT" if complete else "RERISK_CALLS_INCOMPLETE",
        "logical_calls": len(results),
        "valid_results": sum(row["schema_valid_and_locally_validated"] for row in results),
        "usage": usage,
        "next": "ZERO_API_COMPARE_AGAINST_ORIGINAL_RISK",
    }
