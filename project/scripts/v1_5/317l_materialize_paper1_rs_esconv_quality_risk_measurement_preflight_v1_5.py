...
RS ESConv external test, using the EXACT SAME instrument as the EvoEmo
measurement (v1_5_rs_ms_quality_risk_blind_review.py -- same judge
model/config, same prompt templates, same six risk families, same
position-randomized blind pairwise design) for genuine cross-track
comparability, per the user's explicit fairness requirement.
"""
...
ESCONV_TEST_CASES = ROOT / "outputs/pm_v1_5_paper1_rs_esconv_external_test_preflight_20260812/qualification_cases_private.jsonl"
ESCONV_TEST_RESULTS = ROOT / "outputs/pm_v1_5_paper1_rs_esconv_external_test_live_20260812/generator_results_private.jsonl"
RELEASE_MANIFEST = ROOT / "outputs/pm_v1_5_paid_run_release.json"
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_esconv_quality_risk_measurement_preflight_20260812"
STAGE = "paper1_rs_esconv_quality_risk_measurement_v1"
USD_CAP = 0.7

REVIEWER_ID = "RS_ESCONV_QUALITY_RISK_INDEPENDENT_GPT56"
...
def main() -> None:
    if OUT.exists():
        raise RuntimeError("RS ESConv quality/risk measurement preflight exists; refusing overwrite")
    authority = read_json(AUTHORITY)
    current = authority["current_execution_phase"]
    if current["id"] != "RS_ESCONV_EXTERNAL_TEST_EXECUTION":
        raise RuntimeError("RS ESConv external test execution phase is not current")

    cases = {row["case_id"]: row for row in read_jsonl(ESCONV_TEST_CASES)}
    results = read_jsonl(ESCONV_TEST_RESULTS)
    ...
    eligible_case_ids = sorted(
        ...
        if "routed" in kinds and "baseline" in kinds
        and kinds["routed"]["execution_status"] in ("clean", "clean_trace_sanitized")
        and kinds["baseline"]["execution_status"] in ("clean", "clean_trace_sanitized")
    )
    ...
    for case_id in eligible_case_ids:
        ...
        pair_id = "rsesconvqrpair_" + stable_hex(case_id)
        ...
        quality_plan.append({
            "protocol": "pm-v1.5-paper1-rs-esconv-quality-call-v1",
            "logical_call_id": "rsesconvqualcall_" + stable_hex(REVIEWER_ID, quality_id),
            "kind": "quality",
        ...
        for label, reply in (("baseline", baseline["final_reply"]), ("routed", routed["final_reply"])):
            risk_id = "rsesconvrisk_" + stable_hex(pair_id, label, "RISK")
            risk_ids[label] = risk_id
            risk_messages = risk_prompt_messages(item_id=risk_id, dialogue=dialogue, reply=reply)
            risk_items.append({
                "protocol": "pm-v1.5-paper1-rs-esconv-risk-blind-item-v1",
                "blind_item_id": risk_id,
                "dialogue": dialogue,
                "response": reply,
            })
            risk_plan.append({
                "protocol": "pm-v1.5-paper1-rs-esconv-risk-call-v1",
                "logical_call_id": "rsesconvriskcall_" + stable_hex(REVIEWER_ID, risk_id),
                "kind": "risk",
                "reviewer_id": REVIEWER_ID,
                "endpoint_key": ENDPOINT_KEY,
                "item_id": risk_id,
    ...
    checks = {
        "eligible_pairs_present": len(private_map) > 0,
        ...
        "same_instrument_module_as_evoemo_measurement": True,
    ...
    if not all(checks.values()):
        raise RuntimeError(f"RS ESConv quality/risk measurement preflight failed: {checks}")
    ...
    report = {
        "protocol": "pm-v1.5-paper1-rs-esconv-quality-risk-measurement-preflight-v1",
        "status": "PASS_MEASUREMENT_PROPOSAL_READY_HUMAN_APPROVAL_REQUIRED_BEFORE_ANY_API_CALL",
        "checks": checks,
        "design": (
            "Same instrument as the EvoEmo quality/risk measurement (v1_5_rs_ms_quality_risk_blind_review.py, "
            "unmodified): position-randomized blind A/B pairwise quality, absolute independent per-response "
            "risk scoring across the same six families. Scope: all 18 real baseline-vs-routed pairs from the "
            "completed RS ESConv external test where both arms produced valid structured output."
        ),
        "reviewer_id": REVIEWER_ID,
        "proposed_authorization": {
            ...
            "scope": f"{len(quality_plan)} blind pairwise quality calls + {len(risk_plan)} absolute risk calls judging baseline (M0) vs routed (RS) replies from the completed RS ESConv external test.",
        },
        "artifacts": {
        ...
        "source_hashes": {
            ...
            "esconv_test_cases": sha256_file(ESCONV_TEST_CASES),
            "esconv_test_results": sha256_file(ESCONV_TEST_RESULTS),
