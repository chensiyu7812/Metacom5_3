#!/usr/bin/env python3
"""Run the exact fresh-identity MS-only quality/risk blind measurement."""
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_ms_only_quality_risk_measurement_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_ms_only_quality_risk_measurement_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_ms_quality_risk_measurement_endpoints_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_ms_only_quality_risk_measurement_preflight_20260812"
PLAN = PREFLIGHT / "call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_only_quality_risk_measurement_live_20260812"
USD_CAP = 0.7
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "MS_ONLY_QUALITY_RISK_MEASUREMENT_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("MS-only quality/risk measurement execution is not current")
    if bundle["current_phase"]["id"] != "MS_ONLY_QUALITY_RISK_MEASUREMENT_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    dry = {
        "status": f"LIVE_READY_EXACT_{len(calls)}_MS_ONLY_QUALITY_RISK_CALLS",
    ...
    try:
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                ...
                raw.append({
                    "protocol": "pm-v1.5-paper1-ms-only-quality-risk-measurement-raw-v1",
            ...
            results.append({
                "protocol": "pm-v1.5-paper1-ms-only-quality-risk-measurement-result-v1",
                "logical_call_id": call["logical_call_id"],
                "kind": call["kind"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"MS-only quality/risk measurement {len(results)} kind={call['kind']} valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-only-quality-risk-measurement-live-report-v1",
