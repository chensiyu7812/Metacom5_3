#!/usr/bin/env python3
"""Run the exact 24-call source-annotated MS control qualification."""
...
STAGE = "paper1_ms_source_annotated_control_qualification_v1"
AUTHORITY = ROOT / "data/pm_v1_5_contracts/active_method_authority_v1.json"
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_ms_source_annotated_control_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_ms_source_annotated_control_execution_v1.json"
...
PLAN = ROOT / "outputs/pm_v1_5_paper1_ms_source_annotated_control_preflight_20260812/call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_source_annotated_control_live_20260812"
USD_CAP = 0.75
...
def _require_authority() -> dict[str, Any]:
    ...
    expected = {"path": str(PHASE.relative_to(ROOT)), "sha256": sha256_file(PHASE)}
    if current["id"] != "MS_SOURCE_ANNOTATED_CONTROL_EXECUTION" or current["active_phase_manifest"] != expected:
        raise RuntimeError("source-annotated control execution is not current")
    if alias.get("compatibility_alias_of") != "current_execution_phase" or alias["active_phase_manifest"] != expected:
...
def main() -> None:
    ...
    if len(calls) != 24 or len({row["logical_call_id"] for row in calls}) != 24:
        raise RuntimeError("exact 24-call plan required")
    for call in calls:
    ...
    dry = {
        "status": "LIVE_READY_EXACT_24_SOURCE_ANNOTATED_CONTROL_CALLS",
        "logical_calls": 24,
    ...
    try:
        for index, call in enumerate(calls, 1):
            ...
            try:
                ...
                raw.append({
                    "protocol": "pm-v1.5-paper1-ms-source-annotated-control-raw-v1",
            ...
            results.append({
                "protocol": "pm-v1.5-paper1-ms-source-annotated-control-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"source-annotated control {index}/24 valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-source-annotated-control-live-report-v1",
        "status": "CONTROL_CALLS_COMPLETE_ZERO_API_QUALIFICATION_AUDIT_NEXT" if len(results) == 24 else "CONTROL_CALLS_INCOMPLETE",
        "logical_calls": len(results),
        "valid_results": sum(row["schema_valid_and_locally_validated"] for row in results),
        "valid_by_reviewer": {
            reviewer: sum(row["reviewer_id"] == reviewer and row["schema_valid_and_locally_validated"] for row in results)
            for reviewer in phase["execution"]["reviewers"]
        },
        "usage": usage,
        "training_labels_created_or_changed": 0,
        "public_201_calls": 0,
        "pm_fits": 0,
        "next": "ZERO_API_COMPARE_WITH_PRIVATE_CONTROL_KEY_AND_AUDIT_ALL_SOURCE_PLAN_FIELDS",
    }
