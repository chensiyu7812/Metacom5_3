#!/usr/bin/env python3
"""Run the exact 18-call fresh-identity RS EvoEmo opportunity anchor review."""
...
from metacom_pm.v1_5_rs_opportunity_evoemo_review import RSOpportunityEvoEmoReview  # noqa: E402
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_evoemo_opportunity_anchor_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_evoemo_opportunity_anchor_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_evoemo_opportunity_anchor_endpoints_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_rs_evoemo_opportunity_anchor_preflight_20260812"
PLAN = PREFLIGHT / "call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_evoemo_opportunity_anchor_live_20260812"
USD_CAP = 0.15
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_EVOEMO_OPPORTUNITY_ANCHOR_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("RS EvoEmo opportunity anchor execution is not current")
    ...
    if bundle["current_phase"]["id"] != "RS_EVOEMO_OPPORTUNITY_ANCHOR_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    if len(calls) != 18 or len({row["logical_call_id"] for row in calls}) != 18:
        raise RuntimeError("exact 18-call plan required")
    for call in calls:
        if call["messages_sha256"] != sha256_text(canonical_json(call["messages"])):
            raise RuntimeError(f"prompt drift: {call['logical_call_id']}")
        if call["schema_sha256"] != sha256_text(canonical_json(RSOpportunityEvoEmoReview.model_json_schema())):
            raise RuntimeError("schema drift")
    dry = {
        "status": "LIVE_READY_EXACT_18_RS_EVOEMO_ANCHOR_CALLS",
        "logical_calls": 18,
        "maximum_physical_attempts": 36,
        ...
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                result, parsed = clients[endpoint_key].chat(
                    ...
                    seed=int(call["seed"]),
                    response_schema=RSOpportunityEvoEmoReview,
                    retries=2,
                )
                raw.append({
                    "protocol": "pm-v1.5-paper1-rs-evoemo-opportunity-anchor-raw-v1",
                ...
                if parsed.state_id != call["state_id"]:
                    raise ValueError("state_id_mismatch")
                validated = parsed.model_dump(mode="json")
            except Exception as exc:
                error = f"{type(exc).__name__}: {exc}"
            results.append({
                "protocol": "pm-v1.5-paper1-rs-evoemo-opportunity-anchor-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"RS EvoEmo opportunity anchor {len(results)}/18 valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-rs-evoemo-opportunity-anchor-live-report-v1",
        "status": "ANCHOR_CALLS_COMPLETE_ZERO_API_ANALYSIS_NEXT" if complete else "ANCHOR_CALLS_INCOMPLETE",
        ...
        "fits": 0,
        "next": "ZERO_API_COMPARE_AGAINST_ROUTER_DECISIONS",
    }
