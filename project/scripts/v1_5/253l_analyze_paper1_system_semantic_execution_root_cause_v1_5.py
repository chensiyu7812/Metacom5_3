...
def main() -> None:
    ...
    analysis = {
        ...
        "api_calls": 0,
        "headline": "The four-head/16-action design remains correct. Current failure is not general retrieval absence: in the learned-MS-ON RS slice all 7 Rank-1 sources were PI-usable and 6/7 were generator-claimed, yet human verified Function was 0/7. The immediate bottleneck is source-to-response realization and response-act interaction, not a need for another general semantic model.",
        ...
        "minimum_repair_route": [
            ...
            "Use existing 7 learned-ON states for a zero-API R0-vs-RS interaction diagnosis. This tests whether RS crowds out MS without new generation or new labels.",
            "If existing MS+R0 replies contain source-attributable Function, fix joint response planning and retest a tiny frozen slice. If not, run one 8-16-state retrieval-oracle x executor-oracle decomposition before any new head training.",
    ...
    artifact = {
        ...
        "manifest": {
            ...
            "blocks": [
                ...
                {"id": "summary", "type": "markdown", "body": "## 技术结论\n\n**第一篇仍然可以实现，但不能把当前结果叫做完整 PM 及格。** 四头独立判断、组合成16动作、再联合执行的总体逻辑是对的。当前最关键的新证据是：learned MS 开启的7个RS状态里，PI确认7/7来源可用，generator有6/7自报使用，但人评可归因Function是0/7。因此这次不是一般性的‘召回不到’，也不是需要再加一个大语义模型；断点在可用来源如何形成一条可确认、且不压低回复的贡献，以及RS是否把MS挤掉。", "layout": "full", "sourceId": "root_cause_analysis"},
                ...
                {"id": "splice", "type": "markdown", "body": "## 还有没有硬拼接\n\n旧V2代码仍要求exact prior-user statement、词面重合和强制use，仍留在仓库里，是版本路由风险。当前V3实验路径已经删除这些义务：32条MS-ON回复exact source整段复制为0，平均token Jaccard很低；safe non-use保留正常回复，不再因telemetry错误替换成固定句。因此‘当前实验仍在硬拼接’不成立，但‘仓库仍可能误调旧路径’成立。", "layout": "full", "sourceId": "root_cause_analysis"},
                {"id": "harm", "type": "markdown", "body": "## 吸收后反而变差的比例，目前算不出来\n\nPI终值在7个learned-ON状态上是MS+RS 5胜、RS-only 2胜；若只看强制MS臂，变差是2/7=28.6%。但Function为0/7，所以没有任何分母能证明‘记忆已被吸收后导致变差’。这2条只能叫MS请求臂较差，原因可能是生成措辞、RS随机差异或资源干扰，不能归因于成功吸收。", "layout": "full", "sourceId": "root_cause_analysis"},
                {"id": "architecture_text", "type": "markdown", "body": "## 你设想的统一数据、独立四头、联合16动作是正确方案\n\n统一的是state ID、当前上下文、owner group和outer fold；不统一的是四个head的候选、特征和标签。每个state可产生四条component-view训练行，四个binary head分别输出概率，再组合成16种requested action。joint planner必须允许多个记忆同时存在，并将它们组织到一条连贯回复中；cost只在四头输出后的投影/比较中使用。", "layout": "full", "sourceId": "root_cause_analysis"},
                ...
                {"id": "route", "type": "markdown", "body": "## 最短成功路线\n\n1. 先做零API版本正规化和每head的UNCERTAIN_AS_OFF记账。\n2. 用现成7个learned-ON states检查MS+R0是否出现Function，定位RS crowd-out；不重生成。\n3. 若R0仍为0，只做8–16条retrieval-oracle × executor-oracle分解，确认是Rank-1还是实现层。\n4. 只修一个被证实的瓶颈，做极小同栈复测。\n5. 随后进入版本化外部development smoke；冻结后再做ESConv、ES-MemEval、EvoEmo cross-fitted正式报告。\n\n现在不引入新的Hugging Face意图模型。只有oracle分解证明Rank-1是主瓶颈时，才预注册一个cross-encoder reranker挑战BGE-M3；否则加模型只会增加调参面。", "layout": "full", "sourceId": "root_cause_analysis"},
                ...
                {"id": "questions", "type": "markdown", "body": "## 下一轮只回答三个问题\n\n- 同样7个可用来源在MS+R0下，是否能出现人可确认的source-attributable Function？\n- 若不能，retrieval oracle与executor oracle谁能恢复Function？\n- closure/relief状态下，RS head能否稳定OFF并让R0自然收尾？\n\n这三个问题回答后，就不再需要靠版本感觉判断MP/MS/ME‘有没有希望’。", "layout": "full"},
            ],
        },
        "snapshot": {
            "version": 1,
