#!/usr/bin/env python3
"""Execute the exact hash-bound RS+MS EvoEmo external test (real generator calls)."""
...
PROTOCOL = "pm-v1.5-paper1-rs-ms-evoemo-external-test-live-v1"
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_evoemo_external_test_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_evoemo_external_test_execution_v1.json"
...
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_evoemo_external_test_live_20260812"
USD_CAP = 0.15
...
def require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_EVOEMO_EXTERNAL_TEST_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("RS+MS EvoEmo external test execution is not current")
    ...
    if bundle["current_phase"]["id"] != "RS_MS_EVOEMO_EXTERNAL_TEST_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    if preflight_report["status"] != "PASS_EXTERNAL_TEST_PROPOSAL_READY_HUMAN_APPROVAL_REQUIRED_BEFORE_ANY_API_CALL":
        raise RuntimeError("frozen RS+MS EvoEmo external test preflight is not valid")
    for call in calls:
    ...
    try:
        for call in sorted(calls, key=lambda row: (row["is_baseline"] is False, row["case_id"])):
            ...
            if len(completed) % 10 == 0 or len(completed) == len(calls):
                print(f"RS+MS EvoEmo external test {len(completed)}/{len(calls)}", flush=True)
    finally:
        client.close()
    ...
    report = {
        "protocol": PROTOCOL,
        "status": "RS_MS_EXTERNAL_TEST_GENERATION_COMPLETE_PAIRED_BASELINE_MEASUREMENT_REQUIRED" if complete else "RS_MS_EXTERNAL_TEST_GENERATION_INCOMPLETE_RESUME_NEXT_INVOCATION",
        ...
        "next": "ZERO_API_PAIRED_BASELINE_VS_ROUTED_ANALYSIS" if complete else "RERUN_WITH_SAME_RUN_IDENTITY_TO_RESUME",
    }
