#!/usr/bin/env python3
"""Run the exact 23-call fresh-identity RS+MS content-following review."""
...
from metacom_pm.v1_5_rs_ms_content_following_review import RSMSContentFollowingReview  # noqa: E402
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_content_following_review_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_content_following_review_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_ms_content_following_review_endpoints_v1.json"
...
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_content_following_review_live_20260812"
USD_CAP = 0.3
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_CONTENT_FOLLOWING_REVIEW_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("RS+MS content-following review execution is not current")
    ...
    if bundle["current_phase"]["id"] != "RS_MS_CONTENT_FOLLOWING_REVIEW_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    if len(calls) != 23 or len({row["logical_call_id"] for row in calls}) != 23:
        raise RuntimeError("exact 23-call plan required")
    for call in calls:
        if call["messages_sha256"] != sha256_text(canonical_json(call["messages"])):
            raise RuntimeError(f"prompt drift: {call['logical_call_id']}")
        if call["schema_sha256"] != sha256_text(canonical_json(RSMSContentFollowingReview.model_json_schema())):
            raise RuntimeError("schema drift")
    dry = {
        "status": "LIVE_READY_EXACT_23_RS_MS_CONTENT_FOLLOWING_REVIEW_CALLS",
        "logical_calls": 23,
        "maximum_physical_attempts": 46,
        ...
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                result, parsed = clients[endpoint_key].chat(
                    ...
                    seed=int(call["seed"]),
                    response_schema=RSMSContentFollowingReview,
                    retries=2,
                )
                raw.append({
                    "protocol": "pm-v1.5-paper1-rs-ms-content-following-review-raw-v1",
                ...
                if parsed.case_id != call["case_id"]:
                    raise ValueError("case_id_mismatch")
                validated = parsed.model_dump(mode="json")
            except Exception as exc:
                error = f"{type(exc).__name__}: {exc}"
            results.append({
                "protocol": "pm-v1.5-paper1-rs-ms-content-following-review-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"RS+MS content-following review {len(results)}/23 valid={validated is not None}", flush=True)
    finally:
    ...
    complete = len(results) == 23
    report = {
        "protocol": "pm-v1.5-paper1-rs-ms-content-following-review-live-report-v1",
        "status": "REVIEW_CALLS_COMPLETE_ZERO_API_ANALYSIS_NEXT" if complete else "REVIEW_CALLS_INCOMPLETE",
        ...
        "fits": 0,
        "next": "ZERO_API_COMPARE_AGAINST_GENERATOR_CLAIMED",
    }
