#!/usr/bin/env python3
"""Run the exact 24-call fresh-identity MS final-control-repair qualification."""
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_ms_final_control_repair_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_ms_final_control_repair_execution_v1.json"
...
PLAN = ROOT / "outputs/pm_v1_5_paper1_ms_final_control_repair_preflight_20260812/call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_final_control_repair_live_20260812"
USD_CAP = 0.75
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "MS_FINAL_CONTROL_REPAIR_QUALIFICATION_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("final control repair qualification execution is not current")
    ...
    if bundle["current_phase"]["id"] != "MS_FINAL_CONTROL_REPAIR_QUALIFICATION_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    dry = {
        "status": "LIVE_READY_EXACT_24_FINAL_CONTROL_REPAIR_CALLS",
        "logical_calls": 24,
    ...
    try:
        for index, call in enumerate(calls, 1):
            ...
            try:
                ...
                raw.append({
                    "protocol": "pm-v1.5-paper1-ms-final-control-repair-raw-v1",
            ...
            results.append({
                "protocol": "pm-v1.5-paper1-ms-final-control-repair-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"final control repair {index}/24 valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-final-control-repair-live-report-v1",
