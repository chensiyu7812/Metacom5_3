#!/usr/bin/env python3
"""Run the exact 12-call fresh-identity Claude Haiku 4.5 challenger probe."""
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_ms_claude_haiku_challenger_probe_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_ms_claude_haiku_challenger_probe_execution_v1.json"
...
PLAN = ROOT / "outputs/pm_v1_5_paper1_ms_claude_haiku_challenger_probe_preflight_20260812/call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_claude_haiku_challenger_probe_live_20260812"
USD_CAP = 0.75
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "MS_CLAUDE_HAIKU_CHALLENGER_PROBE_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("claude haiku challenger probe execution is not current")
    ...
    if bundle["current_phase"]["id"] != "MS_CLAUDE_HAIKU_CHALLENGER_PROBE_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    dry = {
        "status": "LIVE_READY_EXACT_12_CLAUDE_HAIKU_CHALLENGER_PROBE_CALLS",
        "logical_calls": 12,
    ...
    if OUT.exists():
        # Resume an interrupted attempt at this exact one-shot identity (e.g. a
        # client-side timeout mid-call). Never a fresh directory/identity --
        # only completes calls the authorized plan already covers.
        ...
        if not calls:
            raise RuntimeError("live output already complete; nothing to resume")
    else:
    ...
    try:
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                ...
                raw.append({
                    "protocol": "pm-v1.5-paper1-ms-claude-haiku-challenger-probe-raw-v1",
            ...
            results.append({
                "protocol": "pm-v1.5-paper1-ms-claude-haiku-challenger-probe-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"claude haiku challenger probe {index}/12 valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-claude-haiku-challenger-probe-live-report-v1",
