#!/usr/bin/env python3
"""Run the exact 31-call fresh-identity restatement warm-close requality measurement."""
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_restatement_warm_close_requality_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_restatement_warm_close_requality_execution_v1.json"
ENDPOINTS = ROOT / "configs/paper1_rs_ms_quality_risk_measurement_endpoints_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_requality_preflight_20260812"
PLAN = PREFLIGHT / "call_plan_private.jsonl"
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_requality_live_20260812"
USD_CAP = 0.5
...
def _require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_REQUALITY_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("restatement warm-close requality execution is not current")
    if bundle["current_phase"]["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_REQUALITY_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def main() -> None:
    ...
    for call in calls:
        ...
        if call["schema_sha256"] != sha256_text(canonical_json(RSMSQualityBlindReview.model_json_schema())):
            raise RuntimeError("schema drift")
    dry = {
        "status": "LIVE_READY_EXACT_31_RESTATEMENT_WARM_CLOSE_REQUALITY_CALLS",
        "logical_calls": 31,
        ...
        for index, call in enumerate(calls, len(results) + 1):
            ...
            try:
                result, parsed = clients[endpoint_key].chat(
                    ...
                    seed=int(call["seed"]),
                    response_schema=RSMSQualityBlindReview,
                    retries=2,
                )
                raw.append({
                    "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-requality-raw-v1",
                ...
                if parsed.pair_id != call["item_id"]:
                    raise ValueError("item_id_mismatch")
                validated = parsed.model_dump(mode="json")
            except Exception as exc:
                error = f"{type(exc).__name__}: {exc}"
            results.append({
                "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-requality-result-v1",
                "logical_call_id": call["logical_call_id"],
                "reviewer_id": call["reviewer_id"],
            ...
            print(f"restatement warm-close requality {len(results)}/31 valid={validated is not None}", flush=True)
    finally:
    ...
    report = {
        "protocol": "pm-v1.5-paper1-rs-ms-restatement-warm-close-requality-live-report-v1",
        "status": "REQUALITY_CALLS_COMPLETE_ZERO_API_ANALYSIS_NEXT" if complete else "REQUALITY_CALLS_INCOMPLETE",
        "logical_calls": len(results),
        "valid_results": sum(row["schema_valid_and_locally_validated"] for row in results),
        "usage": usage,
        "next": "ZERO_API_DEBLIND_AND_COMPARE",
    }
