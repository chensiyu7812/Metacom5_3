#!/usr/bin/env python3
"""Execute the exact hash-bound 50-call RS/MS Panel V2 (326l output)."""
...
PROTOCOL = "pm-v1.5-paper1-rs-ms-panel-v2-live-v1"
STAGE = "paper1_rs_ms_panel_v2_v1"
AUTHORITY = ROOT / "data/pm_v1_5_contracts/active_method_authority_v1.json"
BUNDLE = ROOT / "data/pm_v1_5_contracts/paper1_active_execution_bundle_v1.json"
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_panel_v2_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_panel_v2_execution_v1.json"
...
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_panel_v2_live_20260813"
...
USD_CAP = 0.0
STAGE_ID = "RS_MS_PANEL_V2_EXECUTION"
...
def require_authority() -> dict[str, Any]:
    ...
    if current["id"] != STAGE_ID or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("RS/MS Panel V2 execution is not current")
...
def build_messages(
    case: dict[str, Any], arm: str, current_context: str, ms_row: dict[str, Any] | None, rs_decision
) -> list[dict[str, str]]:
    owner = case["runtime_owner_key"]
    candidates: dict[str, V3Candidate | None] = {"MP": None, "MS": None, "ME": None, "RS": None}
    if "MS" in arm.split("+")[0] and ms_row is not None:
    ...
    if arm.split("+")[1] == "RS" and rs_decision is not None and rs_decision.status == "retrieved_top1":
    ...
    mem = "MS" if "MS" in arm.split("+")[0] else "M0"
    action = f"{mem}+{arm.split('+')[1]}"
    plan = build_component_general_plan_v3(requested_action_id=action, current_user_id=owner, candidates=candidates)
...
def main() -> None:
    ...
    ms_by_id = {row["candidate_id"]: row for row in read_jsonl(EVOEMO_CANDIDATES) if row["component"] == "MS"}
    ...
    for case in cases:
        ...
        ms_row = None
        for row in ms_by_id.values():
            if row["runtime_owner_key"] == case["runtime_owner_key"] and row["literal_text"] == case["ms_exact_source"]:
                ms_row = row
                break
        ...
        for arm in case["arms_present"]:
            messages = build_messages(case, arm, current_context, ms_row, rs_decision)
            calls.append(
                {
    ...
    if len(calls) != 50:
        raise RuntimeError(f"exact 50-call plan required, got {len(calls)}")

    dry = {
        "protocol": PROTOCOL,
        "status": "LIVE_READY_EXACT_50_RS_MS_PANEL_V2_CALLS",
    ...
    for call in sorted(calls, key=lambda row: row["physical_call_id"]):
        ...
        for attempt in (1, 2):
            try:
                ...
                print(f"RS/MS panel V2 {len(completed)}/50", flush=True)
                last_error = None
                break
    ...
    summary = {
        "protocol": PROTOCOL,
        "status": f"LIVE_COMPLETE_{len(completed)}_OF_50",
        "run_identity": phase["execution"]["run_identity"],
