#!/usr/bin/env python3
"""Propose (zero-API) the exact 24-call qualification plan for the realistic anchor set.

This is a materially different check from round 2, not a forbidden third
attempt at the same broken instrument: the rubric was rewritten (functional-
risk test, corrected ONE_QUESTION rule), and 5 of 12 items are freshly
grounded in real label patterns rather than invented dilemmas. Fresh reviewer
identities are used regardless (round 2's identities are permanently
consumed). Makes no API calls and spends nothing; a human must still
explicitly approve this exact stage/identity/cost cap before 274l can spend
anything.
"""
...
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_realistic_anchor_qualification_preflight_20260812"
STAGE = "paper1_ms_realistic_anchor_qualification_v1"
USD_CAP = 0.75

REVIEWERS = {
    "MS_ANCHOR_PRIMARY_GPT56": "openai_gpt_5_6_sol",
    "MS_ANCHOR_CHALLENGER_GEMINI": "google_gemini_2_5_flash",
}
...
def main() -> None:
    ...
    if current["id"] != "MS_REALISTIC_ANCHOR_SET_MATERIALIZED_USER_REVIEW_NEXT" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("realistic anchor set materialization is not the current execution phase")
    ...
    for reviewer_id, endpoint_key in REVIEWERS.items():
        ...
        for item in controls:
            ...
            plan.append({
                "protocol": "pm-v1.5-paper1-ms-realistic-anchor-call-v1",
                "logical_call_id": "msanchorcall_" + _stable(reviewer_id, item["repair_item_id"]),
    ...
    checks = {
        ...
        "two_fresh_reviewer_identities_not_retired": REVIEWERS.keys() == {"MS_ANCHOR_PRIMARY_GPT56", "MS_ANCHOR_CHALLENGER_GEMINI"},
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-realistic-anchor-preflight-v1",
        ...
        "proposed_authorization": {
            ...
            "precedent_actual_cost_usd_same_shape_prior_round": 0.193249,
            "scope": (
                "Exactly 24 review calls (12 GPT-5.6 primary + 12 Gemini 2.5 Flash challenger) against "
                "the 12-item realistic anchor set (7 items carried unmodified from round 2, 5 freshly "
                "grounded in real label patterns), using the corrected rubric. No 201-item reannotation, "
                "training-label change, PM fit, threshold change, generator, MP/ME, baseline, or "
                "external experiment."
            ),
        },
        "artifacts": {
