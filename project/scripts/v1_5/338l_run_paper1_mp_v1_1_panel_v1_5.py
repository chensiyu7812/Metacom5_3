#!/usr/bin/env python3
"""Execute the exact hash-bound 15-call MP Panel V1.1 (335l output)."""
...
PROTOCOL = "pm-v1.5-paper1-mp-v1-1-panel-live-v1"
STAGE = "paper1_mp_v1_1_panel_v1"
STAGE_ID = "MP_V1_1_PANEL_EXECUTION"
AUTHORITY = ROOT / "data/pm_v1_5_contracts/active_method_authority_v1.json"
BUNDLE = ROOT / "data/pm_v1_5_contracts/paper1_active_execution_bundle_v1.json"
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_mp_v1_1_panel_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_mp_v1_1_panel_execution_v1.json"
CASES = ROOT / "outputs/pm_v1_5_paper1_mp_v1_1_panel_20260813/development_cases_private.jsonl"
...
OUT = ROOT / "outputs/pm_v1_5_paper1_mp_v1_1_panel_live_20260813"
...
def require_authority() -> dict[str, Any]:
    ...
    if current["id"] != STAGE_ID or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("MP V1.1 panel execution is not current")
...
def rebuild_calls(cases, states_by_id, mp_by_owner_field):
    calls = []
    for case in cases:
        state = states_by_id[case["state_id"]]
        owner = case["runtime_owner_key"]
        field = case["profile_field"]
        ...
        if candidate_row is not None:
            ...
            candidates_map["MP"] = V3Candidate(
                component="MP", evidence_id=candidate_row["candidate_id"], meaning_cue=template.meaning_cue,
                exact_source=candidate_row["literal_text"], owner_id=owner, time_status="STRICTLY_PAST",
        ...
        for arm_name, messages in arms.items():
            if arm_name not in case["arms_present"]:
                continue
            calls.append({"physical_call_id": f"{case['case_id']}::{arm_name}", "case_id": case["case_id"], "state_id": case["state_id"],
                "arm": arm_name, "messages": messages, "messages_sha256": sha256_text(canonical_json(messages)),
...
def main() -> None:
    ...
    mp_by_owner_field: dict[tuple[str, str], Any] = {}
    for row in read_jsonl(EVOEMO_CANDIDATES):
        if row["component"] == "MP":
            mp_by_owner_field[(row["runtime_owner_key"], row["profile_field"])] = row
    calls = rebuild_calls(cases, states_by_id, mp_by_owner_field)
    if len(calls) != 15:
        raise RuntimeError(f"exact 15-call plan required, got {len(calls)}")

    dry = {"protocol": PROTOCOL, "status": "LIVE_READY_EXACT_15_MP_V1_1_CALLS", "logical_calls": len(calls),
    ...
    for call in sorted(calls, key=lambda row: row["physical_call_id"]):
        ...
        for attempt in (1, 2):
            try:
                ...
                print(f"MP v1.1 panel {len(completed)}/15", flush=True)
                last_error = None
                break
    ...
    summary = {"protocol": PROTOCOL, "status": f"LIVE_COMPLETE_{len(completed)}_OF_15", "run_identity": phase["execution"]["run_identity"],
               "completed": len(completed), "total": 15, "output_directory": str(OUT.relative_to(ROOT))}
