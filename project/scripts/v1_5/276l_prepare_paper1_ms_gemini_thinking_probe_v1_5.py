#!/usr/bin/env python3
"""Propose (zero-API) a 12-call Gemini-thinking-enabled challenger probe.

Only the challenger side is re-tested. GPT-5.6 primary already passed
(10/12) in the immediately prior round (paper1_ms_realistic_anchor_qualification_v1,
...
This probe isolates the one real variable under test: does giving the Gemini
challenger a nonzero thinking budget change its systematic bias toward
inventing an UNDERSTANDING framing? Uses a fresh reviewer identity and a
dedicated endpoint config (gemini_thinking_budget: 2048, max_output_tokens:
4096) that does not touch the hash-bound
...
OUT = ROOT / "outputs/pm_v1_5_paper1_ms_gemini_thinking_probe_preflight_20260812"
STAGE = "paper1_ms_gemini_thinking_probe_v1"
USD_CAP = 0.75

REVIEWER_ID = "MS_ANCHOR_CHALLENGER_GEMINI_THINKING"
ENDPOINT_KEY = "google_gemini_2_5_flash_thinking"
...
def main() -> None:
    ...
    if current["id"] != "MS_REVIEWER_MODEL_BIAS_IDENTIFIED_USER_DECISION_NEXT" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("reviewer-model-bias phase is not the current execution phase")
    ...
    for item in controls:
        ...
        plan.append({
            "protocol": "pm-v1.5-paper1-ms-gemini-thinking-probe-call-v1",
            "logical_call_id": "msgeminithinkcall_" + _stable(REVIEWER_ID, item["repair_item_id"]),
    ...
    checks = {
        ...
        "single_reviewer_fresh_identity": REVIEWER_ID == "MS_ANCHOR_CHALLENGER_GEMINI_THINKING",
        ...
        "gemini_thinking_budget_is_nonzero": endpoint["gemini_thinking_budget"] > 0,
    ...
    report = {
        "protocol": "pm-v1.5-paper1-ms-gemini-thinking-probe-preflight-v1",
        ...
        "proposed_authorization": {
            ...
            "scope": (
                "Exactly 12 Gemini 2.5 Flash calls with thinking_budget=2048 (vs 0 previously), "
                "against the same 12-item realistic anchor set, fresh reviewer identity. Tests "
                "whether a nonzero thinking budget fixes the challenger's systematic bias found "
                "in the last 2 rounds. GPT-5.6 primary side is not re-called; its already-real "
                "10/12 result from the prior round is reused for the combined gate."
            ),
        },
        "artifacts": {
