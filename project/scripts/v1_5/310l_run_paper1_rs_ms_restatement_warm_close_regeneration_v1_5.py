#!/usr/bin/env python3
"""Execute the exact hash-bound 31-call restatement warm-close regeneration."""
...
PROTOCOL = "pm-v1.5-paper1-rs-ms-restatement-warm-close-regeneration-live-v1"
...
PHASE = ROOT / "data/pm_v1_5_contracts/paper1_rs_ms_restatement_warm_close_regeneration_execution_phase_v1.json"
CONFIG = ROOT / "configs/paper1_rs_ms_restatement_warm_close_regeneration_execution_v1.json"
PREFLIGHT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_regeneration_preflight_20260812"
...
OUT = ROOT / "outputs/pm_v1_5_paper1_rs_ms_restatement_warm_close_regeneration_live_20260812"
USD_CAP = 0.15
...
def require_authority() -> dict[str, Any]:
    ...
    if current["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_REGENERATION_EXECUTION" or current["active_phase_manifest"] != expected_bundle:
        raise RuntimeError("restatement warm-close regeneration execution is not current")
    if bundle["current_phase"]["id"] != "RS_MS_RESTATEMENT_WARM_CLOSE_REGENERATION_EXECUTION" or bundle["current_phase"]["parent_phase"] != expected_phase:
...
def rebuild_plan(case: dict[str, Any], variant: dict[str, Any]):
...
def main() -> None:
    ...
    for call in calls:
        ...
        messages = response_generation_messages_v3(current_context=case["current_context"], current_goal=CURRENT_GOAL, plan=rebuild_plan(case, variant))
    ...
    dry = {
        "protocol": PROTOCOL,
        "status": "LIVE_READY_EXACT_31_RESTATEMENT_WARM_CLOSE_REGENERATION_CALLS",
        "logical_calls": 31,
    ...
    try:
        for call in sorted(calls, key=lambda row: row["case_id"]):
            ...
            case = cases[call["case_id"]]
            variant = variant_by_card[case["rs_selected_card_id"]]
            plan = rebuild_plan(case, variant)
            ...
            print(f"restatement warm-close regeneration {len(completed)}/31", flush=True)
    finally:
        client.close()
    ...
    complete = len(completed_rows) == 31
    report = {
        "protocol": PROTOCOL,
        "status": "REGENERATION_COMPLETE_ZERO_API_REQUALITY_NEXT" if complete else "REGENERATION_INCOMPLETE",
        "logical_calls_completed": len(completed_rows),
        "execution_status_counts": status_counts,
        "api_calls": 0,
        "pm_refits": 0,
        "next": "ZERO_API_REQUALITY_MEASUREMENT_DESIGN",
    }
