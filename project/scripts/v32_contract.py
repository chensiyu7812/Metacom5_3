...
This module is intentionally small and strict.  All V3.2 scripts should import
action parsing, canonical action construction, schema helpers, and sanitizer
helpers from here instead of reimplementing local variants.
...
SOURCES_TO_ACTION: dict[frozenset[str], str] = {v: k for k, v in ACTION_TO_SOURCES.items()}
MEMORY_ACTIONS: tuple[str, ...] = tuple(ACTION_TO_SOURCES.keys())
ACTION_IDS: tuple[str, ...] = tuple(f"{m}+{s}" for m in MEMORY_ACTIONS for s in STRATEGY_ACTIONS)
...
def canonical_memory_action(sources: Iterable[str]) -> str:
    ...
    try:
        ...
        raise V32ContractError(f"Unsupported memory source set: {sorted(src_set)}") from exc
...
def required_sources(memory_action_or_action_id: str) -> tuple[str, ...]:
    if "+" in memory_action_or_action_id:
        ...
        if memory_action_or_action_id not in ACTION_TO_SOURCES:
            raise V32ContractError(f"Unknown memory action: {memory_action_or_action_id!r}")
        sources = ACTION_TO_SOURCES[memory_action_or_action_id]
    return tuple(src for src in SOURCE_ORDER if src in sources)


def require_exact_keys(obj: Mapping[str, Any], required_keys: Iterable[str], context: str = "object") -> None:
    ...
    if missing or extra:
        raise V32ContractError(f"{context} keys mismatch: missing={missing} extra={extra}")


def strict_int_012(value: Any, context: str = "value") -> int:
    if type(value) is not int or value not in (0, 1, 2):
        raise V32ContractError(f"{context} must be strict int 0|1|2, got {value!r}")
    return value


def strict_int_range(value: Any, lo: int, hi: int, context: str = "value") -> int:
    if type(value) is not int or not (lo <= value <= hi):
        raise V32ContractError(f"{context} must be strict int in [{lo}, {hi}], got {value!r}")
    return value
...
def strict_choice(value: Any, choices: Sequence[str], context: str = "value") -> str:
    if type(value) is not str or value not in set(choices):
        raise V32ContractError(f"{context} must be one of {list(choices)}, got {value!r}")
    return value
...
def strict_source_list(value: Any, context: str = "sources") -> list[str]:
    ...
    for i, src in enumerate(value):
        if type(src) is not str or src not in SOURCE_ORDER:
            raise V32ContractError(f"{context}[{i}] invalid source: {src!r}")
        if src in seen:
            raise V32ContractError(f"{context} duplicate source: {src!r}")
        seen.add(src)
        out.append(src)
    return out
...
def sanitize_memory_store(
    ...
    clean: dict[str, list[dict[str, Any]]] = {src: [] for src in SOURCE_ORDER}
    id_map: dict[str, str] = {}
    idx = 0
    for src in SOURCE_ORDER:
        items = memory_store.get(src) or []
        if not isinstance(items, Sequence):
            raise V32ContractError(f"memory_store[{src}] must be a sequence")
        for item in items:
            if not isinstance(item, Mapping):
                raise V32ContractError(f"memory_store[{src}] item must be object")
            original_id = str(item.get("memory_id") or item.get("id") or f"{src}_{idx}")
            opaque_id = f"mem_{idx:03d}"
            idx += 1
            text = normalize_text(item.get("text"))
            if not text:
                raise V32ContractError(f"memory item {original_id!r} has empty text")
            sanitized = {
                "opaque_id": opaque_id,
                "source": src,
            ...
            if leaked:
                raise V32ContractError(f"sanitized item leaked forbidden fields: {sorted(leaked)}")
            clean[src].append(sanitized)
            id_map[original_id] = opaque_id
    return clean, id_map


def sanitize_selected_memory_ids(ids: Sequence[Any], id_map: Mapping[str, str]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for raw in ids:
        key = str(raw)
        if key not in id_map:
            raise V32ContractError(f"selected memory id not present in sanitized map: {key!r}")
        opaque = id_map[key]
        if opaque in seen:
