...
parser = argparse.ArgumentParser(
    description="Freeze EvoEmo Response Eval V4 evaluator code/config after generation."
)
...
parser.add_argument("--out", type=Path, default=ROOT / "outputs/evoemo_response_v4_eval_freeze.json")
...
data_paths = [
    ...
    ROOT / "outputs/evoemo_selective/dialogues.jsonl",
...
print(
    create_study_freeze(
        ...
        prompt_files=[
            ...
            ROOT / "src/metacom_pm/evo_response_v4.py",
        ],
        out_path=args.out,
        notes={
            "freeze_scope": "evoemo_response_v4_evaluation",
            "generation_freeze_sha256": generation_freeze_sha256,
            "generation_attestation": str(args.generation_attestation),
            "evaluation_policy": (
                "This freeze binds the V4 response evaluator code/configuration "
                "used after EvoEmo selective generation was completed and attested. "
                "It does not authorize regenerating dialogues."
            ),
            "api_budget_policy": (
