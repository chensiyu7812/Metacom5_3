...
23. [ ] 在现有 token/call/latency pilot 余额内，以最多 `$0.40` 运行一个最便宜完整 10-round DG scenario 的 bit-exact upstream retry pilot；只读 stop/length/retry/token/cost。`$0.25` 不能覆盖该 scenario 最坏三次尝试的约 `$0.305` 预留。
