...
（生产40% vs BGE 4%，来自50条子样本），而不是那个被夸大的state级p值**。

下一步该做什么也因此改变：**不是把同一批用户的state数量继续扩大**（那不能解决"只有12个用户"
这个根本限制），而是：(1) 第二独立标注员覆盖至少约20条、必须覆盖全部12个用户和Rank-1不一致
的state，不能只抽某几个用户的子集；(2) 补齐新增88条的双侧4类细分标签；(3) 汇报口径统一换成
用户级胜负和用户聚类bootstrap，不再用普通state级p值当核心证据；(4) 测一下BGE的查询延迟、
（如果加缓存）缓存后延迟、以及本地模型的内存成本，这是"要不要接入生产"决策的另一半，纯统计
显著性不是唯一考量。完成这些之前，`ms_semantic_encoder`保持default-off不变。

## 建议

138条全量结果**方向上支持进一步评估BGE-M3**，但不能像早前版本那样说"已经统计显著确认"：
合并样本下的用户级方向一致性（8/11个用户倾向BGE）和"完全不搭边"比例的量级差距，是目前站得住
脚的证据；state级p≈9.3e-5和95% CI[60.9%,79.3%]因为忽略了用户内相关性，**过度自信，不应
再作为核心论据引用**，用户级的p≈0.227和CI[-0.030,0.530]才是诚实的当前证据强度。在正式冻结前，
仍然需要：(1) 第二个独立标注员，覆盖全部12个用户，不只是随机20%的state；
