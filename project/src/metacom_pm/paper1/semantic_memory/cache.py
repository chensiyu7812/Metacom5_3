"""Content-addressed, atomic success cache for semantic compiler calls."""
...
CACHE_PROTOCOL = "paper1-semantic-memory-success-cache-v1"
...
class SuccessCache:
    ...
    def load(self, identity: CompilerCallIdentity) -> dict[str, Any] | None:
        ...
        if row.get("protocol") != CACHE_PROTOCOL:
            raise RuntimeError(f"semantic compiler cache protocol mismatch: {path}")
        if row.get("identity") != identity.model_dump(mode="json"):
            raise RuntimeError(f"semantic compiler cache identity mismatch: {path}")
        result = row.get("result")
        if not isinstance(result, dict):
            raise RuntimeError(f"semantic compiler cache result is invalid: {path}")
        return result
