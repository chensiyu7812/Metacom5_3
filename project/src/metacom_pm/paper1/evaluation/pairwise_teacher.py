...
PAIRWISE_TEACHER_PROTOCOL = "paper1-pairwise-material-effect-teacher-v2"
VERDICTS = frozenset({"A_better", "B_better", "equivalent", "uncertain"})

DG_REFERENCE_INSTRUCTION = (
    "The task-related historical excerpt is not exhaustive. Use the complete prior "
    "history to check a claimed memory when needed; absence from the excerpt alone "
    "does not establish fabrication. Distinguish seeker statements from supporter "
    "guesses, and past events from current states using dates and visible updates. "
    "If the supplied evidence still cannot support a reliable comparison, choose uncertain."
)
...
def _unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, item in pairs:
        if key in value:
            raise ValueError("duplicate JSON field in teacher output")
        value[key] = item
    return value
